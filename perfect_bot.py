...
ADMIN_PASSWORD_HASH = generate_password_hash('your_password')  # Пароль для дашборда (замени)
...
CRYPTO_CURRENCIES = {
    'биткоин': 'BTC', 'биткоины': 'BTC', 'биткоина': 'BTC', 'btc': 'BTC',
    'эфир': 'ETH', 'эфириум': 'ETH', 'эфира': 'ETH', 'eth': 'ETH',
    'рипл': 'XRP', 'риплы': 'XRP', 'xrp': 'XRP',
    'догекоин': 'DOGE', 'доге': 'DOGE', 'догекоина': 'DOGE', 'doge': 'DOGE',
    'кардано': 'ADA', 'карданы': 'ADA', 'ada': 'ADA',
    'солана': 'SOL', 'соланы': 'SOL', 'sol': 'SOL',
    'лайткоин': 'LTC', 'лайткоины': 'LTC', 'ltc': 'LTC'
}

# Глобальные переменные
CACHE = {}  # Кэш курсов валют
CACHE_TIMEOUT = 300  # 5 минут
STATS = {  # Статистика в памяти
    "users": {},
    "total_requests": 0,
    "request_types": {},
    "subscriptions": {},
...
def get_exchange_rate(from_currency, to_currency, amount=1):
    ...
    from_code = CRYPTO_CURRENCIES.get(from_key, FIAT_CURRENCIES.get(from_key, from_key.upper()))
    to_code = CRYPTO_CURRENCIES.get(to_key, FIAT_CURRENCIES.get(to_key, to_key.upper()))
    
    try:
        if from_code in CRYPTO_CURRENCIES.values() or to_code in CRYPTO_CURRENCIES.values():
            url = f'https://api.coingecko.com/api/v3/simple/price?ids={from_code.lower()}&vs_currencies={to_code.lower()}'
            response = requests.get(url, timeout=5).json()
            rate = response[from_code.lower()][to_code.lower()]
        else:
        ...
        logger.info(f"Fetched rate for {from_code} to {to_code}: {rate}")
        return result, rate
    except Exception as e:
        logger.error(f"Error fetching rate for {from_code} to {to_code}: {e}")
        return None, "Не удалось получить курс. Проверь валюты."
...
async def subscribe(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if response.get("ok"):
            ...
            logger.error(f"Invoice creation failed for {user_id}: {response}")
    ...
    try:
        ...
        if response.get("ok") and response["result"]["items"]:
            ...
            if status == "paid":
                ...
                await update.message.reply_text("Оплата ещё не подтверждена. Проверь статус в @Send и попробуй снова.")
        else:
